...
class CryptoCardOrderDetails extends StatelessWidget {
  const CryptoCardOrderDetails({
    super.key,
    ...
    return Column(
      children: [
        ...
        TwoColumnCell(
          ...
          value: shortTxhashFrom(transactionListItem.cryptoCardOrderInfo?.transactionId ?? ''),
            ...
            onTap: () async {
              ...
              await sRouter.push(
                CryptoCardTransactionHistoryRoute(
                  cardId: transactionListItem.cryptoCardOrderInfo?.cardId ?? '',
...
class _CryptoCardOrderDetailsHeader extends StatelessWidget {
  const _CryptoCardOrderDetailsHeader({
    required this.transactionListItem,
  });
  ...
  Widget build(BuildContext context) {
    final paymentAsset = getIt<FormatService>().findCurrency(
      assetSymbol: transactionListItem.cryptoCardOrderInfo?.paymentAssetId ?? '',
    );

    final fromAmount = transactionListItem.cryptoCardOrderInfo?.paymentAmount ?? Decimal.zero;
