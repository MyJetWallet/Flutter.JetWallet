...
class CryptoCardRefundDetails extends StatelessWidget {
  const CryptoCardRefundDetails({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        _CryptoCardRefundDetailsHeader(
        ...
        TwoColumnCell(
          ...
          value: shortTxhashFrom(transactionListItem.cryptoCardRefundInfo?.transactionId ?? ''),
            ...
            onTap: () async {
              ...
              await sRouter.push(
                CryptoCardTransactionHistoryRoute(
                  cardId: transactionListItem.cryptoCardRefundInfo?.cardId ?? '',
...
class _CryptoCardRefundDetailsHeader extends StatelessWidget {
  const _CryptoCardRefundDetailsHeader({
    required this.transactionListItem,
  });
  ...
  Widget build(BuildContext context) {
    final paymentAsset = getIt<FormatService>().findCurrency(
      assetSymbol: transactionListItem.cryptoCardRefundInfo?.paymentAssetId ?? '',
    );

    final fromAmount = transactionListItem.cryptoCardRefundInfo?.paymentAmount ?? Decimal.zero;
